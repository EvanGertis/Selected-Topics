...
<script type="text/javascript">$(init);$( window ).unload(function() {removeStorage.removeItem("someVarKey1");});function reset() {  var someVarName = true;sessionStorage.setItem("someVarKey1", someVarName);window.location.reload();}function init() {	document.getElementById('resetButton').style.display = 'none';document.getElementById("resetButton").style.visibility = "hidden";if (false && sessionStorage.getItem("someVarKey1"))$("#one").focus();var numbers = [3, 4, 5, 1, 2];initialize(numbers);}</script>  <script>  answer = "Loop Body: is the part of the body that contains the statements to be repeated.  Iteration: is one time execution of the loop body.Loop Continuation Condition: is a Boolean expression that controls the execution of the loop.Infinite Loop: is a loop that runs forever due to an error in the code.Off-by-one: is an error in the program that causes the loop body to be executed one more or less time."  
